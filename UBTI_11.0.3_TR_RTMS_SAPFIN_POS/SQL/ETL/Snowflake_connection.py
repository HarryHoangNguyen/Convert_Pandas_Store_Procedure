...
def read_local_config():
    """Read configuration file from the current directory"""
    current_dir = os.path.dirname(os.path.abspath(__file__))
    user_config_file = os.path.join(current_dir, 'config.cfg')

    if not os.path.exists(user_config_file):
        print(f"Configuration file not found at: {user_config_file}")
        return None

    cfg = configparser.ConfigParser()
    cfg.read(user_config_file)
    print(f"Configuration loaded successfully from: {user_config_file}")
    return cfg
