...
def get_logger(etl_name):
    ...
    log_folder = os.path.join(base_path, "logs", today.strftime("%Y%m%d%H%M"))
    parquet_folder = os.path.join(base_path, "parquets", today.strftime("%Y%m%d%H%M"))
    
    # create directories if they don't exist
    os.makedirs(log_folder, exist_ok=True)
    os.makedirs(parquet_folder, exist_ok=True)

    log_file = os.path.join(log_folder, f"{etl_name}_{today.strftime('%Y%m%d%H%M')}.log")
    # Create logger
