...
def write_to_snowflake(dataframe, connection, database, schema, table_name, procdate,logger):
    try:
        ...
        cur.execute(f"SELECT COUNT(*) FROM {database}.{schema}.{table_name.upper()}")
