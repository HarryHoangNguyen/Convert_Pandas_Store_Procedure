...
if not procdate.startswith('2'):
    # Default procdate to yesterday's date in 'YYYYMMDD' format
    procdate = (pd.Timestamp.now() - pd.Timedelta(days=1)).strftime('%Y%m%d')
...
try:
    write_to_snowflake(df_V2_TB_RTMS_RTShopCloud, connection, database = 'SPPL_DEV_DWH', schema = 'SPPL_PUBLIC', table_name = 'SP_UBT_GETRTSHOPCLOUD', procdate=procdate,logger = logger)
...
print(df_V2_TB_RTMS_RTShopCloud)
