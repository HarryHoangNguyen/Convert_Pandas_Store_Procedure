...
##############################################################################
    ...
    df_V2_TSNWagerSalesAmountData = (
        ...
        .assign(
            WAGER=lambda x: (pd.to_numeric(x['WAGER'], errors='coerce').fillna(0) / 100).round(2) * 100,
            SALES=lambda x: (pd.to_numeric(x['SALES'], errors='coerce').fillna(0) / 100).round(2) * 100
        )
        .rename(columns={'LOCTYPEID':'LOCTYPE'})[[
    ...
    # Initialize df_V2_LocationInvoicePeriodData as empty DataFrame
    ...
    if vNowDayName in (0, 4):
        ...
        # Create merged dataframe first
        df_merged = df_V2_TerminalForInvoicePeriodData.merge(
            df_ztubt_terminal_location,
            left_on='TERDISPLAYID',
            right_on='TERDISPLAYID'
        )

        # Filter using the merged dataframe's FLAG column
        df_V2_LocationInvoicePeriodData = df_merged.loc[df_merged['FLAG'].isin(['FUN', 'REC'])]\
            .groupby(['LOCID', 'LOCDISPLAYID', 'LOCNAME', 'LOCTYPEID', 'ISIBG', 'ISGST', 'PRODUCTNAME', 'FLAG', 'TRANSTYPE'])\
            .agg({'AMOUNT': 'max'}).reset_index()



        # Filter for other flags (NOT FUN, REC) with sum aggregation
        df_temp = df_merged.loc[~df_merged['FLAG'].isin(['FUN', 'REC'])]\
            .groupby(['LOCID', 'LOCDISPLAYID', 'LOCNAME', 'LOCTYPEID', 'ISIBG', 'ISGST', 'PRODUCTNAME', 'FLAG', 'TRANSTYPE'])\
            .agg({'AMOUNT': 'sum'}).reset_index()
        df_V2_LocationInvoicePeriodData = pd.concat([df_V2_LocationInvoicePeriodData, df_temp], ignore_index=True)

        df_V2_LocationInvoicePeriodData.rename(columns={
            'LOCID': 'LOCID',
            'LOCDISPLAYID': 'LOCDISPLAYID',
            'LOCNAME': 'LOCNAME',
            'LOCTYPEID': 'LOCTYPE',
            'ISIBG': 'ISIBG',
            'ISGST': 'ISGST',
    ...
    ## Calculate IBG Amount for IBG Retailer
        df_V2_Tab = df_V2_LocationInvoicePeriodData[
            (df_V2_LocationInvoicePeriodData['ISIBG'] == True) &
            (df_V2_LocationInvoicePeriodData['LOCTYPE'].isin([2, 4]))
        ]
        print( df_V2_Tab.columns.tolist())
        df_V2_Tab['AMOUNT_CLC'] = df_V2_Tab['AMOUNT'].where(
            (df_V2_Tab['FLAG'] == 'COL') & (df_V2_Tab['TYPE'] != 'OO') |
            (df_V2_Tab['FLAG'].isin(['CAN', 'PAY', 'RFD', 'RBT']) & (df_V2_Tab['TYPE'] != 'OO')) |
            (df_V2_Tab['FLAG'] == 'SAL') |
            (df_V2_Tab['FLAG'] == 'GST')
        ).fillna(0)
        df_temp = df_V2_Tab.groupby(['LOCID'], as_index=False).agg(
            AMOUNT_CLC=('AMOUNT_CLC', 'sum')
        ).reset_index()
        vTotalAmount = df_temp.loc[df_temp['AMOUNT_CLC'] < 0, 'AMOUNT_CLC'].sum()
    ...
    ## MANUAL PAYMENT ENDED DD/MM/YYYY-NON IBG RETAILERS
        ...
        df_V2_TempAmountPerLocation = df_V2_LocationInvoicePeriodData[
            (df_V2_LocationInvoicePeriodData['ISIBG'] != True) &
            (df_V2_LocationInvoicePeriodData['LOCTYPE'].isin([2, 4]))
        ]

        df_V2_TempAmountPerLocation['AMOUNT_CLC'] = df_V2_TempAmountPerLocation['AMOUNT'].where(
            (df_V2_TempAmountPerLocation['FLAG'] == 'COL') & (df_V2_TempAmountPerLocation['TYPE'] != 'OO') |
            (df_V2_TempAmountPerLocation['FLAG'].isin(['CAN', 'PAY', 'RFD', 'RBT']) & (df_V2_TempAmountPerLocation['TYPE'] != 'OO')) |
            (df_V2_TempAmountPerLocation['FLAG'] == 'SAL') |
            (df_V2_TempAmountPerLocation['FLAG'] == 'GST')
        ).fillna(0)
        df_V2_TempAmountPerLocation = df_V2_TempAmountPerLocation.groupby(['LOCID', 'LOCDISPLAYID', 'LOCNAME'], as_index=False).agg(
            AMOUNT=('AMOUNT_CLC', 'sum')
        ).reset_index()
